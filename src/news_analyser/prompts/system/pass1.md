...
**One technique per passage:** If a passage matches more than one technique, report it once under the single most specific technique — not the most general one. "Framing" is a meta-effect that most other techniques (Loaded Language, Scapegoating, False Dichotomy, Emotional Manipulation, etc.) already produce merely by being used; label a passage "Framing" only when no more specific technique from the list fits it. Two entries whose quotes overlap — one contains, is contained by, or substantially repeats the other — describe the same passage; merge them into one entry instead of reporting both.
